...
<div class="page" style="padding: 20px;">
    ...
    <div class="invoice-date">
        ...
        <strong>Reference No:</strong> {{ $referenceNumber ?? 'BZ-' . date('YmdHis') }}
    </div>
    ...
    <div class="invoice-details">
        ...
        <table class="invoice-table">
            ...
            <tr>
                ...
                <td>{{ $business ?? $productName ?? 'N/A' }} {{ isset($scale) ? '- ' . $scale : '' }}</td>
            </tr>
            <tr>
                <td><strong>Net Loan (Selling Price):</strong></td>
                <td>${{ number_format(($netLoan ?? $sellingPrice ?? $amount ?? 0) / 1.06, 2) }}</td>
            </tr>
            <tr>
                <td><strong>Bank Admin Fee (6%):</strong></td>
                <td>${{ number_format(($bankAdminFee ?? (($amount ?? 0) - (($amount ?? 0) / 1.06))), 2) }}</td>
            </tr>
            <tr class="invoice-row-highlight">
                <td><strong>Gross Loan (incl. 6% admin fee):</strong></td>
                <td class="invoice-total">${{ number_format($grossLoan ?? $amount ?? 0, 2) }}</td>
            </tr>
        </table>
        ...
        <table class="invoice-table">
            ...
            <tr>
                ...
                <td>{{ $creditTerm ?? 'N/A' }} months</td>
            </tr>
            <tr>
                <td><strong>Monthly Payment:</strong></td>
                <td class="invoice-total">${{ number_format($monthlyPayment ?? 0, 2) }}</td>
            </tr>
            <tr>
                <td><strong>First Payment Date:</strong></td>
                <td>{{ isset($firstPaymentDate) ? date('F Y', strtotime($firstPaymentDate)) : date('F Y', strtotime('first day of next month')) }}</td>
            </tr>
            <tr>
                <td><strong>Last Payment Date:</strong></td>
                <td>{{ isset($lastPaymentDate) ? date('F Y', strtotime($lastPaymentDate)) : date('F Y', strtotime('+' . ($creditTerm ?? 12) . ' months')) }}</td>
            </tr>
        </table>
    </div>

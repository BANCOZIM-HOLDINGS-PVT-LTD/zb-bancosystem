...
<div class="page" style="padding: 30px;">
    ...
    <div class="invoice-container">
        ...
        <table class="invoice-product-table">
            ...
            <tbody>
                <tr>
                    <td>{{ $formResponses['creditFacilityType'] ?? $formResponses['business'] ?? $formResponses['productName'] ?? $formResponses['loanPurpose'] ?? $formResponses['category'] ?? $get('productName') ?? $get('category') ?? '' }}</td>
                    <td>{{ $formResponses['productCode'] ?? $get('productCode') ?? '' }}</td>
                    <td>${{ number_format((float)str_replace(',', '', $formResponses['loanAmount'] ?? $formResponses['amount'] ?? $formResponses['finalPrice'] ?? $formResponses['netLoan'] ?? $formResponses['sellingPrice'] ?? '0'), 2) }}</td>
                </tr>
                <tr>
                    <td></td>
                    <td></td>
